...
```bash
$ touch bower.json && echo '{"name":"myProject"}' > bower.json # bower.json 파일을 만들고, 빈 JSON 오브젝트를 써 놓는다.
...
css vendor prefix, css minify, js uglify 까지 하면, 아주 쪼금이라도 성능이 향상될 수 있다.
