...
**`참고`** 가령, 사용자가 'http://api.example.com/v1/articles' 란 인터넷 Resource 를 요청하면, DNS (==Domain Name Service) 에서 IP 주소로 번역하는 과정을 거쳐 컴퓨터간에 통신을 하게 된다. 그런데, 운영체제에 포함된 'hosts' 파일은 DNS 로 요청이 나가기 전에 낚아 채는 역할을 한다. 즉 'hosts' 파일 안에 사용자가 요청한 도메인에 해당하는 레코드가 있으면 'hosts' 파일의 내용이 DNS 를 오버라이드한다. 쓸데 없는 얘기긴한데... 보통 인터넷을 통해 라이센스 인증을 받는 상용소프트웨어의 경우, 이 hosts 파일을 이용해서 인증 서버에 해당하는 도메인을 로컬 주소로 바꾸고, 로컬 서버에서 인증된 것 처럼 꾸며 어둠의 소프트웨어를 사용할 수 있게 한다.
